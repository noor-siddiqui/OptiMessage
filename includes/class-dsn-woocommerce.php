...
class DSN_WooCommerce {
	...
	public function register_checkout_fields() {
		...
		if ( function_exists( 'woocommerce_register_additional_checkout_field' ) ) {
			...
			woocommerce_register_additional_checkout_field(
				array(
					'id'       => 'dsn/sms_consent',
					'label'    => __( 'I want to receive SMS notifications about my order.', 'desishad-sms-notifier' ),
					'location' => 'contact',
					'type'     => 'checkbox',
	...
	public function legacy_add_checkout_consent_checkbox() {
		...
		woocommerce_form_field( 'dsn_sms_consent', array(
			'type'     => 'checkbox',
	...
	public function legacy_checkout_process() {
		if ( ! function_exists( 'woocommerce_register_additional_checkout_field' ) && get_option( 'dsn_consent_checkout', 0 ) && get_option( 'dsn_consent_required', 0 ) ) {
			if ( empty( $_POST['dsn_sms_consent'] ) ) {
				wc_add_notice( __( 'Please check the SMS consent box to proceed.', 'desishad-sms-notifier' ), 'error' );
			}
		}
	}

	public function block_save_checkout_consent( $order, $request ) {
		if ( ! get_option( 'dsn_consent_checkout', 0 ) ) return;
	...
	private function has_consent( $order ) {
		if ( get_option( 'dsn_send_no_consent', 0 ) ) return true;
		...
		if ( $order_consent === true || $order_consent === '1' || $order_consent === 'yes' ) return true;
		if ( $order_consent === false || $order_consent === '0' || $order_consent === 'no' ) return false;
	...
	public function trigger_order_placed( $order_id, $order ) {
		if ( ! $this->has_consent( $order ) ) return;
	...
	private function send_notification( $order, $template ) {
		if ( ! get_option( 'dsn_wc_sms_enabled', 1 ) ) return;
