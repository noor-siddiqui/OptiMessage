...
class DSN_Send_SMS {
	...
	private function render_bulk_filter_form() {
		...
		if ( false === $map ) {
			...
			if ( $orders ) {
				// Optimization: wc_get_orders does not automatically prime the user cache like WP_User_Query.
				// We collect and prime unique customer IDs here to prevent N+1 queries during get_user_meta calls in the loop below.
				$customer_ids = array();
