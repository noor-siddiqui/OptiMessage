...
class DSN_Subscribers_List_Table extends WP_List_Table {
	...
	public function no_items() {
		_e( 'No subscribers found.', 'desishad-sms-notifier' );
	}

	public function get_columns() {
		return array(
			'name'        => __( 'Full Name', 'desishad-sms-notifier' ),
			'email'       => __( 'Email', 'desishad-sms-notifier' ),
			'phone'       => __( 'Phone', 'desishad-sms-notifier' ),
			'valid_phone' => __( 'Valid Phone?', 'desishad-sms-notifier' ),
			'address'     => __( 'Address', 'desishad-sms-notifier' ),
			'state'       => __( 'State', 'desishad-sms-notifier' ),
			'consent'     => __( 'SMS Consent', 'desishad-sms-notifier' ),
		);
	}
		...
		if ( isset( $_POST['s'] ) && ! empty( $_POST['s'] ) ) {
			...
			if ( is_email( $search ) ) {
				...
				$args['meta_query'] = array(
					...
					array( 'key' => 'billing_phone', 'value' => $clean, 'compare' => 'LIKE' )
				);
			} else {
				...
				foreach ( $terms as $term ) {
					...
					$meta_query[] = array( 'key' => 'first_name', 'value' => $term, 'compare' => 'LIKE' );
					$meta_query[] = array( 'key' => 'last_name',  'value' => $term, 'compare' => 'LIKE' );
					$meta_query[] = array( 'key' => 'billing_first_name', 'value' => $term, 'compare' => 'LIKE' );
					$meta_query[] = array( 'key' => 'billing_last_name',  'value' => $term, 'compare' => 'LIKE' );
				}
...
class DSN_Guest_Customers_List_Table extends WP_List_Table {
	...
	public function no_items() {
		_e( 'No guest orders found.', 'desishad-sms-notifier' );
	}

	public function get_columns() {
		return array(
			'order_id'    => __( 'Order ID', 'desishad-sms-notifier' ),
			'name'        => __( 'Billing Name', 'desishad-sms-notifier' ),
			'email'       => __( 'Email', 'desishad-sms-notifier' ),
			'phone'       => __( 'Phone', 'desishad-sms-notifier' ),
			'valid_phone' => __( 'Valid Phone?', 'desishad-sms-notifier' ),
			'address'     => __( 'Address', 'desishad-sms-notifier' ),
		...
		if ( isset( $_POST['s'] ) && ! empty( $_POST['s'] ) ) {
			...
			if ( is_email( $search ) ) {
				...
				$args['field_query'] = array(
					'relation' => 'OR',
					array( 'field' => 'billing_phone', 'value' => $search, 'compare' => 'LIKE' ),
					array( 'field' => 'billing_phone', 'value' => $clean, 'compare' => 'LIKE' )
				);
			} else {
				$terms = explode( ' ', $search );
				...
				foreach ( $terms as $term ) {
					$term = trim( $term );
					if ( empty( $term ) ) continue;
					$field_query[] = array( 'field' => 'billing_first_name', 'value' => $term, 'compare' => 'LIKE' );
					$field_query[] = array( 'field' => 'billing_last_name',  'value' => $term, 'compare' => 'LIKE' );
				}
...
class DSN_Subscribers_Tab {
	...
	public function render() {
		...
		echo '<h2>' . __( 'Subscribers & User Consent', 'desishad-sms-notifier' ) . '</h2>';
		echo '<p class="description">' . __( 'Manage all SMS subscribers natively without crashing high-performance databases. Guests who do not create accounts are stored in Guest Orders.', 'desishad-sms-notifier' ) . '</p>';

		echo '<h3 class="nav-tab-wrapper" style="margin-bottom: 20px;">';
		echo '<a href="?page=dsn-settings&tab=subscribers&sub_tab=registered" class="nav-tab ' . ( $sub_tab == 'registered' ? 'nav-tab-active' : '' ) . '">' . __( 'Registered Users', 'desishad-sms-notifier' ) . '</a>';
		echo '<a href="?page=dsn-settings&tab=subscribers&sub_tab=guests" class="nav-tab ' . ( $sub_tab == 'guests' ? 'nav-tab-active' : '' ) . '">' . __( 'Guest Orders', 'desishad-sms-notifier' ) . '</a>';
		echo '</h3>';
		...
		if ( $sub_tab === 'guests' ) {
			...
			$table->prepare_items();
			$table->search_box( __( 'Search Guests', 'desishad-sms-notifier' ), 'search_id' );
			$table->display();
		} else {
			$table = new DSN_Subscribers_List_Table();
			$table->prepare_items();
			$table->search_box( __( 'Search Users', 'desishad-sms-notifier' ), 'search_id' );
			$table->display();
		}
