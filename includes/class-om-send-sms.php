...
class OM_Send_SMS {
	...
	public function ajax_setup_csv_queue() {
		...
		// Validate each number via Twilio Lookup API in batches.
		// ⚡ BOLT OPTIMIZATION: Replacing O(n) synchronous API calls with batched async lookups.
		// Measuring shows this reduces total CSV processing time significantly by processing multiple numbers concurrently.
		$valid_phones   = array();
		$invalid_phones = array();

		// Convert raw_phones into the request format needed by lookup_phone_batch.
		$lookup_requests = array();
		foreach ( $raw_phones as $phone => $user_id ) {
			$lookup_requests[ $phone ] = array( 'phone' => $phone );
		}

		// Process in batches of 50 to avoid timeouts/rate limits.
		$batches = array_chunk( $lookup_requests, 50, true );
		foreach ( $batches as $batch ) {
			$results = OM_Twilio_API::lookup_phone_batch( $batch );
			foreach ( $batch as $phone => $req ) {
