		...
		if ( false === $map ) {
			...
			if ( $orders ) {
				...
				if ( ! empty( $customer_ids ) ) {
					...
					$ids_csv    = implode( ',', array_map( 'intval', $unique_ids ) );

					// ⚡ Bolt: Targeted query to avoid memory bloat from loading all user meta.
					// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Table name and IDs are securely mapped to integers.
					// phpcs:disable WordPress.DB.PreparedSQL.NotPrepared -- The IN clause is securely mapped to integers above.
					$results = $wpdb->get_results( "SELECT user_id, meta_value FROM {$wpdb->usermeta} WHERE meta_key = 'optimessage/sms-consent' AND user_id IN ({$ids_csv})" );
