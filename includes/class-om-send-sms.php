...
class OM_Send_SMS {
	...
	public function ajax_setup_csv_queue() {
		...
		// Validate each number via Twilio Lookup API in batches to prevent timeouts.
		$valid_phones   = array();
		$invalid_phones = array();

		$batch_size    = 50;
		$phone_batches = array_chunk( array_keys( $raw_phones ), $batch_size );

		foreach ( $phone_batches as $batch ) {
			$lookup_requests = array();
			foreach ( $batch as $phone ) {
				$lookup_requests[ $phone ] = array(
					'phone'   => $phone,
					'country' => '', // CSV numbers should include country code or fall back to store default.
				);
			}
