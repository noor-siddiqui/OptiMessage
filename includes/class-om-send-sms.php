...
class OM_Send_SMS {
	...
	public function ajax_setup_csv_queue() {
		...
		// Format requests for batch lookup.
		...
		// Process lookups concurrently in chunks to prevent timeouts.
		$chunks = array_chunk( $lookup_requests, 50, true );
		foreach ( $chunks as $chunk ) {
			$batch_results = OM_Twilio_API::lookup_phone_batch( $chunk );

			foreach ( $batch_results as $original_phone => $result ) {
				if ( is_array( $result ) && ! empty( $result['valid'] ) ) {
					// Use the E.164 formatted number from Twilio.
					$valid_phones[ $result['formatted'] ] = $raw_phones[ $original_phone ];
				} else {
					$invalid_phones[] = $original_phone;
				}
			}
		}
