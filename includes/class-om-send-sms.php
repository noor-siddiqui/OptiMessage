		...
		if ( false === $map ) {
			...
			if ( $orders ) {
				...
				if ( ! empty( $customer_ids ) ) {
						...
						// ⚡ Bolt Optimization: Targeted query to fetch only the required meta key
						// This prevents memory bloat from loading all user metadata when we only need sms-consent.
						// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQL.NotPrepared
						$placeholders = implode( ',', array_map( 'intval', $unique_customer_ids ) );
						$results      = $wpdb->get_results(
							"SELECT user_id, meta_value FROM {$wpdb->usermeta} WHERE meta_key = 'optimessage/sms-consent' AND user_id IN ($placeholders)"
						);
						// phpcs:enable
