...
class OM_Send_SMS {
	...
	private function render_bulk_filter_form() {
		...
		if ( false === $map ) {
			...
			if ( $orders ) {
				// Pre-load targeted user meta cache to avoid N+1 queries and memory bloat during the loop.
				$customer_ids = array();
				...
				if ( ! empty( $customer_ids ) ) {
					...
					$results = $wpdb->get_results( "SELECT user_id, meta_value FROM {$wpdb->usermeta} WHERE meta_key = 'optimessage/sms-consent' AND user_id IN ({$ids_list})" );
