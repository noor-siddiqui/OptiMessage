...
## 2024-05-18 - External API calls in WP_List_Table rendering
**Learning:** The OptiMessage plugin executed synchronous API calls to Twilio (`OM_Twilio_API::lookup_phone`) during the rendering of `OM_Guest_Customers_List_Table` and `OM_Subscribers_List_Table` in the WordPress admin. This caused severe N+1 performance bottlenecks and page load timeouts when displaying unverified phone numbers because it blocked on network requests for every item.
**Action:** Decouple Twilio lookups during admin list table rendering to asynchronous background tasks. Enqueue `as_enqueue_async_action` (Action Scheduler) or fallback to `wp_schedule_single_event` (WP Cron) with checks to prevent scheduling duplicate tasks for the same resource.
