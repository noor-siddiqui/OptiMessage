...
**Action:** When creating new async jobs, ensure the corresponding handler logic is implemented.
\n## 2026-04-24 - Batched Asynchronous HTTP Calls vs. Synchronous Loops
**Learning:** Sequential synchronous API calls (e.g., `OM_Twilio_API::lookup_phone`) in bulk operations like CSV processing cause significant bottlenecks and can lead to PHP process timeouts. The codebase has `curl_multi_init` implemented in `OM_Twilio_API::lookup_phone_batch` which mitigates this.
**Action:** Always chunk arrays for bulk operations and process them concurrently using batched methods like `lookup_phone_batch` instead of looping synchronously over each item. Use `array_chunk($array, $size, true)` to preserve string keys mapping to original items.
