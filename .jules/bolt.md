...
## 2024-05-18 - Synchronous API calls in list table rendering loops
**Learning:** Calling synchronous external APIs like Twilio inside the `column_default` switch block of a `WP_List_Table` (such as `OM_Guest_Customers_List_Table` and `OM_Subscribers_List_Table`) introduces severe N+1 latency, blocking the UI rendering for every row missing validation.
**Action:** Decouple these lookups into asynchronous background jobs (`as_enqueue_async_action` or `wp_schedule_single_event`) during list rendering. Return a visually appropriate status like "Pending Validation" instead of waiting for the external response. Always check for an already scheduled job before enqueuing to prevent duplicate background processes per row.
